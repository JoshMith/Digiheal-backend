...
import { predictDuration } from "../services/ml.service";
...
// Prediction endpoint before making appointment - no authentication required for this  
static predictAppointmentDuration = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        // Basic validation
        if (!department || !priority || !appointmentType) {
          res.status(400).json({
            success: false,
            error:
              "Missing required fields: department, priority, appointmentType",
          });
          return;
        }

        const prediction = await predictDuration({
          department,
          priority,
          appointmentType,
  ...
  static createAppointment = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      // Get ML prediction for suggested duration
      const mlPrediction = await predictDuration({
        department: validatedData.department,
        priority: validatedData.priority,
        appointmentType: validatedData.type,
        symptomCount: 1, // Default, could be enhanced
      });
      ...
      const appointment = await prisma.appointment.create({
        data: {
          ...
          appointmentDate: validatedData.appointmentDate,
          appointmentTime: validatedData.appointmentTime,
          duration: validatedData.duration, // Use user-provided or consider using mlPrediction.predictedDuration
          department: validatedData.department,
      ...
      res.status(201).json({
        ...
        data: {
          ...appointment,
          mlPrediction, // Include ML prediction in response
        },
  ...
  static getAppointmentById = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
  ...
  // Update appointment
  static updateAppointment = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const id = req.params.id as string;
  ...
  static checkInAppointment = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const id = req.params.id as string;
      ...
      res.status(200).json({
        ...
        data: updatedAppointment,
        message:
          "Checked in successfully. Your queue number is " +
  ...
  // Cancel appointment
  static cancelAppointment = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const id = req.params.id as string;
  ...
  static getTodayAppointments = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
  ...
  static getAvailableSlots = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const availableSlots = allSlots.filter(
        (slot) =>
          !bookedSlots.some((booked) => booked.appointmentTime === slot),
      );
  ...
  static getStaffAppointments = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
  ...
  static startAppointment = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const id = req.params.id as string;
  ...
  static completeAppointment = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const id = req.params.id as string;
  ...
  static getAppointmentStats = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const today = new Date();
      today.setHours(0, 0, 0, 0);

      const [totalToday, completedToday, pendingToday, byDepartment, byStatus] =
        await Promise.all([
